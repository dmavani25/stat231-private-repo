...
## Data Collection Process and Nomenclature used

I used Google calendar for all of my time-keeping and I updated it as time progressed. I imported my 14-day calendar data from 09-12-2021 to 09-25-2021 with Google calendar in `ical` format.
...
```{r plotting-data-viualization-1, eval = TRUE}
...
We can conclude that the time devoted in sleeping for me is perfectly consistent statistically because the mean is same as the value for any specific day. Also, we can say that I am more consistent in meditation and sports compared to career related work and procrastination. 

Moreover, we can see the average time spent row at the very bottom of the table for a big-picture estimate of time spent doing each activity per day. On an average, the descending order of time spent doing the given activity each day is:
