...
```{r scatterplot1.1, eval = TRUE, fig.width = 8, fig.height = 4.5}
library(mdsr)
data(CIACountries)
...
```{r scatterplot1.2, eval = TRUE, fig.width = 8, fig.height = 4.5}
library(mdsr)
data(CIACountries)

# define the plot object
g <- ggplot(data = CIACountries, 
            aes(y = gdp, x = educ)) +
  ylab("Gross Domestic Product (GDP)") + xlab("% of GDP spent on education")
...
```{r scatterplot1.3, eval = TRUE, fig.width = 8, fig.height = 4.5}
library(mdsr)
data(CIACountries)
...
```{r scatterplot1.4, eval = TRUE, fig.width = 8, fig.height = 4.5}
library(mdsr)
data(CIACountries)
...
The "x = reorder(drg, mean_charge)" makes sure that the mean_charge data is in ascending order to make the visualization easy and more clean. If we replace "x = reorder(drg, mean_charge)" by "x = drg", then we will get bars which are not in ascending order as generated in the figure below.
...
Now, the graph's x-axis value labels are written in horizontal manner, which makes them overlap on each other and more difficult to read. The theme() line in this graph serves a purpose of setting a theme for labels on x axis to increase their comprehension while visulaizing data.
...
1. Can you please explain the wtd.quantile?

2. Can you please elaborate on %+% and similar syntax?
